...
if ($_REQUEST['event'] == 'ONIMBOTMESSAGEADD') {
   ...
   $arReport['attach'][] = array("MESSAGE" => 'Как разберетесь с этими задачами, просто спросите еще раз [send=Профиль]Профиль[/send] и я дам новую сводку!');

   // send answer message
   $result = restCommand('imbot.message.add', 
      array(
         "DIALOG_ID" => $_REQUEST['data']['PARAMS']['DIALOG_ID'],
         "MESSAGE"   => $arReport['title'] . "\n" . $arReport['report'] . "\n" . 'http://dev.1c-bitrix.ru/bitrix/templates/1c-bitrix-new/images/logo.png',
         "ATTACH"    => array_merge(
            $arReport['attach']
         ),
      ), 
...
function getAnswer($command = '', $user) {
   $arResult = b24BadTasks($user, strtolower($command));
   return $arResult;
}

function b24BadTasks ($user, $search_content) {
   $tasks = restCommand('tasks.task.list', 
      array(
         'filter' => array('RESPONSIBLE_ID' => $user, '%TITLE' => $search_content),
         'select' => array('TITLE', 'RESPONSIBLE_ID')
      ), 
      $_REQUEST["auth"]);
   file_put_contents(__DIR__ . '/log.txt', print_r( $tasks, true));
   if (count($tasks['result']['tasks']) > 0) {
      $arTasks = array();
      foreach ($tasks['result']['tasks'] as $id => $arTask) {
         $arTasks[] = array(
            'LINK' => array(
               'NAME' => $arTask['title'],
               'LINK' => 'https://'.$_REQUEST['auth']['domain'].'/company/personal/user/'.$arTask['responsible']['id'].'/tasks/task/view/'.$arTask['id'].'/'
            )
         );
         $arTasks[] = array(
            'DELIMITER' => array(
